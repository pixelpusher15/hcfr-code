...
void CDataSetDoc::PerformSimultaneousMeasures ( int nMode )
{
	...
	switch ( nMode )
	{
		...
		case 3:
			 ...
			 GenerateSaturationColors (GetColorReference(), GenColors, nSteps, true, false, false, GetConfig()->m_GammaOffsetType, GetMeasure()->GetActiveSatLevel(), GetConfig()->GetUse10bitLevels());
		...
		case 4:
			 ...
			 GenerateSaturationColors (GetColorReference(), GenColors, nSteps, false, true, false, GetConfig()->m_GammaOffsetType, GetMeasure()->GetActiveSatLevel(), GetConfig()->GetUse10bitLevels() );
		...
		case 5:
			 ...
			 GenerateSaturationColors (GetColorReference(), GenColors, nSteps, false, false, true, GetConfig()->m_GammaOffsetType, GetMeasure()->GetActiveSatLevel(), GetConfig()->GetUse10bitLevels() );
		...
		case 6:
			 ...
			 GenerateSaturationColors (GetColorReference(), GenColors, nSteps, true, true, false, GetConfig()->m_GammaOffsetType, GetMeasure()->GetActiveSatLevel(), GetConfig()->GetUse10bitLevels() );
		...
		case 7:
			 ...
			 GenerateSaturationColors (GetColorReference(), GenColors, nSteps, false, true, true, GetConfig()->m_GammaOffsetType, GetMeasure()->GetActiveSatLevel(), GetConfig()->GetUse10bitLevels() );
		...
		case 8:
			 ...
			 GenerateSaturationColors (GetColorReference(), GenColors, nSteps, true, false, true, GetConfig()->m_GammaOffsetType, GetMeasure()->GetActiveSatLevel(), GetConfig()->GetUse10bitLevels() );
		...
		case 9:
			...
			switch (GetConfig()->m_CCMode)
			{
			 ...
			 nMaxSteps = nSteps;
			 mType [ 0 ] = nPattern;
			 GenerateCC24Colors (GetColorReference(), GenColors, GetConfig()->m_CCMode, GetConfig()->m_GammaOffsetType, GetConfig()->GetUse10bitLevels() );
		...
		case -3:
			 ...
			 for (i = 0; i < nSteps ; i ++ )
			 {
				GenIRE [ i ] = GetMeasure () -> GetGrayPercent ( i, GetConfig () -> m_bUseRoundDown, GetConfig()->GetUse10bitLevels());
		...
		default:
			 ...
			 for (i = 0; i < nSteps ; i ++ )
			 {
				GenIRE [ i ] = (int)GetMeasure () -> GetGrayPercent ( i, GetConfig () -> m_bUseRoundDown, GetConfig()->GetUse10bitLevels());
				bIRE [ i ] = GetMeasure () -> m_bIREScaleMode;
				GenColors [ i ] = ColorRGBDisplay(GenIRE[i]);
 			 	mType [ i ] = CGenerator::MT_IRE;
			 }
...
void CDataSetDoc::WaitKey(int Anim)
{
	...
	while ( !bKeyTyped )
	{
		if (Anim != 0 && GetConfig()->GetProfileInt("GDIGenerator","DisplayMode",DISPLAY_DEFAULT_MODE) != DISPLAY_ccast)// && GetConfig()->GetProfileInt("GDIGenerator","DisplayMode",DISPLAY_DEFAULT_MODE) != DISPLAY_rPI) //on/off patterns
		{
			if (OFF)
			{
				switch (Anim)
...
void CDataSetDoc::ComputeGammaAndOffset(double * Gamma, double * Offset, int ColorSpace,int ColorIndex,int Size, bool m_bBT1886)
{
	...
	if ((Size > 2) && (whitelvl > 0))
	{
		...
		case 7: //BBC hgl
			...
			for (int i=0; i<Size; i++)
			{
				x = GetMeasure () -> GetGrayPercent ( i, GetConfig () -> m_bUseRoundDown, GetConfig()->GetUse10bitLevels());
				v = GrayLevelToGrayProp(x, GetConfig () -> m_bUseRoundDown, GetConfig()->GetUse10bitLevels());
			...
			for (int i=1; i<Size-2; i++)
			{
				if ( lumlvl[i] > 0.0 )
				{
                    ...
                    if (m_bBT1886 || mode > 5) // !GetConfig()->m_useMeasuredGamma)
                        avg += log(getL_EOTF(valx[i], GetMeasure()->GetGray(Size -1), GetMeasure()->GetGray(0), GetConfig()->m_GammaRel, GetConfig()->m_Split, mode, GetConfig()->m_DiffuseL, GetConfig()->m_MasterMinL, GetConfig()->m_MasterMaxL, GetConfig()->m_TargetMinL, GetConfig()->m_TargetMaxL,GetConfig()->m_useToneMap, FALSE, GetConfig()->m_TargetSysGamma, GetConfig()->m_BT2390_BS, GetConfig()->m_BT2390_WS, GetConfig()->m_BT2390_WS1))/log(valx[i]);
                    else
	                    if (mode == 5)// && !GetConfig()->m_useMeasuredGamma)
			                avg += log(getL_EOTF(valx[i], GetMeasure()->GetGray(Size -1), GetMeasure()->GetGray(0), GetConfig()->m_GammaRel, GetConfig()->m_Split, mode, GetConfig()->m_DiffuseL, GetConfig()->m_MasterMinL, GetConfig()->m_MasterMaxL, GetConfig()->m_TargetMinL, GetConfig()->m_TargetMaxL,GetConfig()->m_useToneMap, FALSE, GetConfig()->m_TargetSysGamma, GetConfig()->m_BT2390_BS, GetConfig()->m_BT2390_WS, GetConfig()->m_BT2390_WS1) / 100.)/log(valx[i]);
						else
...
void CDataSetDoc::OnMeasurePrimaries() 
{
	...
	if (!m_YWhite && (GetConfig()->m_colorStandard == HDTVa || GetConfig()->m_colorStandard == HDTVb || GetConfig()->m_colorStandard == UHDTV3 || GetConfig()->m_colorStandard == UHDTV4) && GetConfig()->m_GammaOffsetType == 4)
