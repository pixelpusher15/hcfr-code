...
CNearWhiteGrapher::CNearWhiteGrapher()
{
	...
	m_graphCtrl.SetXAxisProps((LPSTR)(LPCSTR)GetConfig()->m_PercentGray, 1, 10, 100);
	m_graphCtrl.SetYAxisProps(m_abY?" cd m-2":m_showL?"":"%", 1, 10, 4000);
	...
	m_logGraphCtrl.SetXAxisProps((LPSTR)(LPCSTR)GetConfig()->m_PercentGray, 1, 10, 100);
...
//	    	CColor Black = pDoc -> GetMeasure () -> GetGray ( 0 );
			...
			double x = ArrayIndexToGrayLevel ( pDoc->GetMeasure()->m_NearWhiteClipCol - size + i, 101, GetConfig () -> m_bUseRoundDown, GetConfig()->GetUse10bitLevels(), GetConfig()->GetRGB16_235());
...
//					val = min(val, GetConfig()->m_TargetMaxL);
				}
				else
...
void CNearWhiteGrapher::AddPointtoLumGraph(int ColorSpace,int ColorIndex,int Size,int PointIndex,CDataSetDoc *pDataSet,long GraphID,long LogGraphID = -1)
{
	...
	if (ColorSpace == 0) 
	{
		...
		if (pDataSet->GetMeasure()->GetOnOffWhite().isValid())
			whitelvl=pDataSet->GetMeasure()->GetOnOffWhite().GetY();
		else if (pDataSet->GetMeasure()->GetNearWhite(Size-1).isValid())
			whitelvl=pDataSet->GetMeasure()->GetNearWhite(Size-1).GetLuxOrLumaValue(GetConfig () -> m_nLuminanceCurveMode);
		else
			whitelvl=100.;
	...
	{
		m_logGraphCtrl.AddPoint(LogGraphID, GrayLevelToGrayProp( (double)(PointIndex+pDataSet->GetMeasure()->m_NearWhiteClipCol-Size), GetConfig () -> m_bUseRoundDown, GetConfig()->GetUse10bitLevels(), GetConfig()->GetRGB16_235()) * 100., log(colorlevel/max)/log(GrayLevelToGrayProp( (double)(PointIndex+101-Size), GetConfig () -> m_bUseRoundDown, GetConfig()->GetUse10bitLevels(), GetConfig()->GetRGB16_235())), lpMsg);
	}
	...
	if (!m_showL && !m_abY)
	{
		...
		if (ColorSpace == 1 && ColorIndex == 1)
	        m_graphCtrl.AddPoint(GraphID, GrayLevelToGrayProp( (double)(PointIndex+pDataSet->GetMeasure()->m_NearWhiteClipCol-Size), GetConfig () -> m_bUseRoundDown, GetConfig()->GetUse10bitLevels(), GetConfig()->GetRGB16_235()) * 100., (max>0?(colorlevel/max)*100.0:0.0), lpMsg, whitelvl);
		else
	        m_graphCtrl.AddPoint(GraphID, GrayLevelToGrayProp( (double)(PointIndex+pDataSet->GetMeasure()->m_NearWhiteClipCol-Size), GetConfig () -> m_bUseRoundDown, GetConfig()->GetUse10bitLevels(), GetConfig()->GetRGB16_235()) * 100., (max>0?(colorlevel/max)*100.0:0.0), lpMsg, NULL);
	}
	else 
	{
		if (m_showL)
	        m_graphCtrl.AddPoint(GraphID, GrayLevelToGrayProp( (double)(PointIndex+pDataSet->GetMeasure()->m_NearWhiteClipCol-Size), GetConfig () -> m_bUseRoundDown, GetConfig()->GetUse10bitLevels(), GetConfig()->GetRGB16_235()) * 100., Y_to_L(whitelvl?colorlevel/whitelvl:colorlevel/max), lpMsg, NULL);
		else
	        m_graphCtrl.AddPoint(GraphID, GrayLevelToGrayProp( (double)(PointIndex+pDataSet->GetMeasure()->m_NearWhiteClipCol-Size), GetConfig () -> m_bUseRoundDown, GetConfig()->GetUse10bitLevels(), GetConfig()->GetRGB16_235()) * 100., Y_to_abY(colorlevel, 1.0), lpMsg, NULL);
...
//	m_Grapher.m_graphCtrl.SetXScale ( GetDocument()->GetMeasure()->m_NearWhiteClipCol - GetDocument()->GetMeasure()->GetNearWhiteScaleSize(), 100);//GetDocument()->GetMeasure()->m_NearWhiteClipCol );
//	m_Grapher.m_logGraphCtrl.SetXScale ( GetDocument()->GetMeasure()->m_NearWhiteClipCol - GetDocument()->GetMeasure()->GetNearWhiteScaleSize(), 100);// GetDocument()->GetMeasure()->m_NearWhiteClipCol );
...
void CNearWhiteHistoView::OnUpdate(CView* pSender, LPARAM lHint, CObject* pHint) 
{
	...
	if ( lHint == UPD_EVERYTHING || lHint == UPD_GRAYSCALEANDCOLORS || lHint == UPD_GRAYSCALE || lHint == UPD_NEARBLACK || lHint == UPD_NEARWHITE || lHint == UPD_DATAREFDOC || lHint == UPD_REFERENCEDATA || lHint == UPD_ARRAYSIZES || lHint == UPD_FREEMEASUREAPPENDED || (lHint >= UPD_REALTIME && lHint != UPD_DISPLAYPROFILE && lHint != UPD_REALTIME + 13))
	{
...
void CNearWhiteHistoView::OnLumGraphL() 
{
	...
	m_Grapher.m_graphCtrl.SetYAxisProps(m_Grapher.m_showL?"":"%", m_Grapher.m_showL?10:10, 0, 20);
	GetConfig()->WriteProfileInt("Near White Histo","Show L",m_Grapher.m_showL);
	GetConfig()->WriteProfileInt("Near White Histo","Absolute Y",m_Grapher.m_abY);
	OnUpdate(NULL,NULL,NULL);
    OnGraphScaleFit();
}

void CNearWhiteHistoView::OnLumGraphYab() 
{
	...
	GetConfig()->WriteProfileInt("Near White Histo","Absolute Y",m_Grapher.m_abY);
	GetConfig()->WriteProfileInt("Near White Histo","Show L",m_Grapher.m_showL);
	m_Grapher.m_graphCtrl.SetYAxisProps(m_Grapher.m_abY?" cd m-2 ":m_Grapher.m_abY?"":"%", 10, 0, 10);
	OnUpdate(NULL,NULL,NULL);
	OnGraphScaleFit();
}
