...
protected:
	...
	// What the cached pair was computed from -- CMeasure's profile generation
	// counter (the cube's contents) and the reference's white and primaries.
	// m_volDirty says "look again", this says whether looking would change
	// anything; without it a sweep re-walks the whole cube on every repaint for
	// the same number.
	enum { VOL_KEY_LEN = 13 };
