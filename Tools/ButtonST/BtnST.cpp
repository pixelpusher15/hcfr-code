...
{
	if (m_bIsFocused) 
	{ // Line added - Fix from codeproject
...
//		m_pbmpOldBk = m_dcBk.SelectObject(&m_bmpBk);  Fix from code project
...
{
	m_crColors[BTNST_COLOR_BK_IN]		= FxGetSysColor(COLOR_BTNFACE);
	m_crColors[BTNST_COLOR_FG_IN]		= FxGetSysColor(COLOR_BTNTEXT);
	m_crColors[BTNST_COLOR_BK_OUT]		= FxGetSysColor(COLOR_BTNFACE);
	m_crColors[BTNST_COLOR_FG_OUT]		= FxGetSysColor(COLOR_BTNTEXT);
	m_crColors[BTNST_COLOR_BK_FOCUS]	= FxGetSysColor(COLOR_BTNFACE);
	m_crColors[BTNST_COLOR_FG_FOCUS]	= FxGetSysColor(COLOR_BTNTEXT);
...
void CButtonST::DrawTransparent(BOOL bRepaint)
{
	...
	{
//		m_dcBk.SelectObject(m_pbmpOldBk);  Fix from codeproject
		::SelectObject(m_dcBk, (HBITMAP)m_pbmpOldBk);
	} // if
...
//		m_pbmpOldBk = m_dcBk.SelectObject(&m_bmpBk);  fix from codeproject
		m_pbmpOldBk = (HBITMAP)::SelectObject(m_dcBk, (HBITMAP)m_bmpBk);
...
DWORD CButtonST::OnDrawBorder(CDC* pDC, CRect* pRect)
{
	...
	if (m_bIsPressed)
	{
		...
		{
			if (m_bDrawBorder)
				pDC->Draw3dRect(pRect, FxGetSysColor(COLOR_BTNSHADOW), FxGetSysColor(COLOR_BTNHILIGHT));
		}
		else    
		{
	...
	else // ...else draw non pressed button
	{
		...
		if (m_bIsFlat)
		{
			if (m_bMouseOnButton && m_bDrawBorder)
				pDC->Draw3dRect(pRect, FxGetSysColor(COLOR_BTNHILIGHT), FxGetSysColor(COLOR_BTNSHADOW));
		}
		else
		{
