...
CGridCtrl::CGridCtrl(int nRows, int nCols, int nFixedRows, int nFixedCols)
{
	...
	m_bShowAlwaysHScroll  = FALSE;
	m_bShowAlwaysVScroll  = FALSE;
...
void CGridCtrl::ValidateAndModifyCellContents(int nRow, int nCol, LPCTSTR strText)
{
    ...
    if (SendMessageToParent(nRow, nCol, GVN_BEGINLABELEDIT) >= 0)
    {
		...
		// Georges GALLERAND: test removed to allow symmetric begin-end notifications
...
void CGridCtrl::EnableScrollBars(int nBar, BOOL bEnable /*=TRUE*/)
{
	...
	{
		if (bEnable || m_bShowAlwaysHScroll)
		{
			if (!IsVisibleHScroll())
			{
	...
	{
		if (bEnable || m_bShowAlwaysVScroll)
		{
			if (!IsVisibleVScroll())
			{
...
void CGridCtrl::ResetScrollBars()
{
    ...
    */
    ...
    SCROLLINFO si;
    si.cbSize = sizeof(SCROLLINFO);
    si.fMask = SIF_PAGE | SIF_RANGE | (m_bShowAlwaysHScroll ? SIF_DISABLENOSCROLL : 0);
    si.nPage = (m_nHScrollMax>0)? VisibleRect.Width() : 0;
    si.nMin = 0;
...
void CGridCtrl::EnsureVisible(int nRow, int nCol)
{
    ...
    */
    ...
    while (rectCell.right > rectWindow.right
		...
		&& GetScrollPos32(SB_HORZ)<m_nHScrollMax // Georges GALLERAND: added to prevent infinite loop
        )
    {
        int nOldPos = GetScrollPos32(SB_HORZ);
		SendMessage(WM_HSCROLL, SB_LINERIGHT, 0);

        if ( nOldPos == GetScrollPos32(SB_HORZ) )
		{
			// Didn't scroll: stop infinite loop
			break;
		}
    ...
    while (rectCell.bottom > rectWindow.bottom
		...
		&& GetScrollPos32(SB_VERT)<m_nVScrollMax // Georges GALLERAND: added to prevent infinite loop
        )
    {
        int nOldPos = GetScrollPos32(SB_VERT);
        SendMessage(WM_VSCROLL, SB_LINEDOWN, 0);

        if ( nOldPos == GetScrollPos32(SB_VERT) )
		{
			// Didn't scroll: stop infinite loop
			break;
		}
