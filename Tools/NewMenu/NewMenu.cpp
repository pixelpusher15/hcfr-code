     ...
     if(bMenuAbove && nMenuHeight)
  {
       if((rcTBItem.bottom+nMenuHeight)<rect.bottom)
    {
     ...
     if(bMenuAbove)
     {
       ...
       if(IsMirroredWnd(pWndCommand->GetSafeHwnd()))
       {
        /* GGA: removed last parameter, not accepted on old Windows platforms */
		return pMenu->TrackPopupMenu(TPM_LEFTBUTTON|TPM_BOTTOMALIGN|TPM_RIGHTALIGN|uFlags, rcTBItem.left, rcTBItem.top,pWndCommand,NULL/*rcTBItem*/);
       }
       else
	   {
        /* GGA: removed last parameter, not accepted on old Windows platforms */
        return pMenu->TrackPopupMenu(TPM_LEFTBUTTON|TPM_BOTTOMALIGN|uFlags, rcTBItem.left, rcTBItem.top,pWndCommand,NULL/*rcTBItem*/);
	   }
	 }
     else
     {
	   ...
	   {
        /* GGA: removed last parameter, not accepted on old Windows platforms */
        return pMenu->TrackPopupMenu(TPM_LEFTBUTTON|TPM_RIGHTALIGN|uFlags, rcTBItem.left, rcTBItem.bottom,pWndCommand,NULL/*rcTBItem*/);
       }
       else
	   {
        /* GGA: removed last parameter, not accepted on old Windows platforms */
        return pMenu->TrackPopupMenu(TPM_LEFTBUTTON|uFlags, rcTBItem.left, rcTBItem.bottom,pWndCommand,NULL/*rcTBItem*/);
       }
     }
  }
}
  ...
  case CNewMenu::STYLE_XP_2003_NOBORDER :
    {
      ...
      {
       menuBarColor = menuBarColor2 = FxGetSysColor(COLOR_3DFACE);
...
COLORREF GetXpHighlightColor()
{
  if(bHighContrast)
  {
    return FxGetSysColor(COLOR_HIGHLIGHT);
  }

  if (NumScreenColors() > 256)
  {
    return MidColor(FxGetSysColor(COLOR_WINDOW),FxGetSysColor(COLOR_HIGHLIGHT));
  }
...
COLORREF CNewMenu::GetMenuColor(HMENU hMenu)
{
  ...
  if(IsShellType()==WinXP)
  {
    ...
    {
      return FxGetSysColor(COLOR_MENUBAR);
    }
  }
  return FxGetSysColor(COLOR_MENU);
}
    ...
    COLORREF *pColor)
{
  // Fx add
  if(fxUseCustomColor)
	  if ( FxLocalGetThemeColor(hTheme,iPartId,iStateId,iColorId,pColor) )
		  return TRUE;
  // End of fx add
...
void CNewMenu::GetMenuBarColor2003(COLORREF& color1, COLORREF& color2, BOOL bBackgroundColor /* = TRUE */)
{
  ...
  if(IsShellType()==Win95 || IsShellType()==WinNT4)
  {
    color1 = color2 = FxGetSysColor(COLOR_MENU);
    return;
  }
  ...
  {
    COLORREF colorWindow = DarkenColor(10,FxGetSysColor(COLOR_WINDOW));
    COLORREF colorCaption = FxGetSysColor(COLOR_ACTIVECAPTION);
  ...
  else
  {
    if (IsMenuThemeActive())
    {
      ...
      //if (pWnd)
      {
        ...
        if(hTheme)
        {
		  ...
		  // Fx mod
		  if(fxUseCustomColor)  // if custom colors do not change selected colors
		  {
			  color1 = colorCaption;
              if (bBackgroundColor)
			  	color2 = LightenColor(110,color1);
    ...
    {
      color1 = ::FxGetSysColor(COLOR_MENU);
      color2 = ::FxGetSysColor(COLOR_WINDOW);
...
void CNewMenu::DrawMenuTitle(LPDRAWITEMSTRUCT lpDIS, BOOL bIsMenuBar)
{
  ...
  COLORREF colorWindow = FxGetSysColor(COLOR_WINDOW);
  COLORREF colorMenuBar = GetMenuColor();

  COLORREF colorLeft = MixedColor(colorWindow,colorMenuBar);
  COLORREF colorRight = ::FxGetSysColor(COLOR_ACTIVECAPTION);
  COLORREF colorText = ::FxGetSysColor(COLOR_CAPTIONTEXT);
  ...
  {
    pDC->Draw3dRect(rect,FxGetSysColor(COLOR_3DSHADOW),FxGetSysColor(COLOR_3DHILIGHT));
  }
  ...
  if(pMenuData->m_nTitleFlags&MFT_LINE)
  {
    if(pMenuData->m_nTitleFlags&MFT_SIDE_TITLE)
    {
      ...
      pDC->Draw3dRect(rect2,FxGetSysColor(COLOR_3DSHADOW),FxGetSysColor(COLOR_3DHILIGHT));
    ...
    else
    {
      ...
      pDC->Draw3dRect(rect2,FxGetSysColor(COLOR_3DHILIGHT),FxGetSysColor(COLOR_3DSHADOW));
...
void CNewMenu::DrawItem_WinXP(LPDRAWITEMSTRUCT lpDIS, BOOL bIsMenuBar)
{
  ...
  COLORREF colorBorder = FxGetSysColor(COLOR_HIGHLIGHT);//DarkenColor(128,colorMenuBar);

  if(bHighContrast)
  {
    colorBorder = FxGetSysColor(COLOR_BTNTEXT);
  }

  if (NumScreenColors() <= 256)
  {
    colorBitmap = FxGetSysColor(COLOR_BTNFACE);
  }
  ...
  //CPen Pen(PS_SOLID,0,FxGetSysColor(COLOR_HIGHLIGHT));
  ...
  if(bIsMenuBar && (lpDIS->itemState&ODS_SELECTED) )
  {
    if(!(lpDIS->itemState&ODS_INACTIVE) )
    {
      ...
      if(!(lpDIS->itemState&ODS_HOTLIGHT) )
      {
        ...
        Pen.DeleteObject();
        colorBorder = bHighContrast?FxGetSysColor(COLOR_BTNTEXT):DarkenColor(128,GetMenuBarColor());
        ...
        if (NumScreenColors() <= 256)
        {
          DWORD clr = FxGetSysColor(COLOR_BTNSHADOW);
          if(lpDIS->itemState&ODS_DRAW_VERTICAL)
          {
            int winW = rect.Width();
            for (Y=0; Y<=1 ;Y++)
  ...
  if( nFlags & MF_SEPARATOR )
  {
    ...
    else
    {
      ...
      pDC->FillSolidRect(rect,FxGetSysColor(COLOR_GRAYTEXT));
    }
  }
  else
  {
    ...
    if(!strText.IsEmpty())
    {
        ...
        (bIsMenuBar && lpDIS->itemState&ODS_INACTIVE) )
      {
        ...
        OldTextColor = pDC->SetTextColor(FxGetSysColor(COLOR_GRAYTEXT));
      }
      else
      {
        ...
        {
          OldTextColor = pDC->SetTextColor(FxGetSysColor(COLOR_WINDOWTEXT));
        }
        else
        {
          OldTextColor = pDC->SetTextColor(FxGetSysColor(COLOR_MENUTEXT));
        }
      }
        ...
        else
        {
          ...
          if (state&ODS_CHECKED)
          {
            ...
            if(selectedflag)
            {
              if (NumScreenColors() > 256)
              {
                pDC->FillSolidRect(rect,MixedColor(colorSel,FxGetSysColor(COLOR_HIGHLIGHT)));
              }
              else
              {
                pDC->FillSolidRect(rect,colorSel); //FxGetSysColor(COLOR_HIGHLIGHT)
              }
            }
            else
            {
              pDC->FillSolidRect(rect,MixedColor(colorBitmap,FxGetSysColor(COLOR_HIGHLIGHT)));
            }
...
void CNewMenu::DrawItem_XP_2003(LPDRAWITEMSTRUCT lpDIS, BOOL bIsMenuBar)
{
  ...
  //COLORREF colorCaption = FxGetSysColor(COLOR_ACTIVECAPTION); //RGB(0,84,227);
  ...
  // Fx mod
	if(fxUseCustomColor)
	{
      colorSel = LightenColor(50,FxGetSysColor(COLOR_HIGHLIGHT));
	  colorCheck = FxSaturateColor(200,FxGetSysColor(COLOR_HIGHLIGHT));
	  colorCheckSel = DarkenColor(50,colorCheck);
	}
  // End of fx mod
  if(!IsMenuThemeActive() &&
      ...
      GetMenuDrawMode()!=STYLE_COLORFUL)
  {
    ...
    colorCheckSel = MixedColor(colorSel,FxGetSysColor(COLOR_HIGHLIGHT));
...
void CNewMenu::DrawItem_SpecialStyle (LPDRAWITEMSTRUCT lpDIS, BOOL bIsMenuBar)
{
  ...
  {
    colorBack = FxGetSysColor(COLOR_HIGHLIGHT);
  ...
  if(!(lpDIS->itemState & ODS_GRAYED))
  {
    if(lpDIS->itemState&(ODS_SELECTED|ODS_HOTLIGHT))
    {
      crTextColor = FxGetSysColor(COLOR_HIGHLIGHTTEXT);
    }
    else
    {
      crTextColor = FxGetSysColor(COLOR_MENUTEXT);
    }
  }
  else
  {
    crTextColor = FxGetSysColor(COLOR_GRAYTEXT);
  }
...
void CNewMenu::DrawItem_Icy(LPDRAWITEMSTRUCT lpDIS, BOOL bIsMenuBar)
{
  ...
  COLORREF colorGrayText = DarkenColor(100,colorMenu);//FxGetSysColor(COLOR_GRAYTEXT);
  COLORREF colorHilight = FxGetSysColor(COLOR_HIGHLIGHT);
  COLORREF colorGrayed = bHighContrast?FxGetSysColor(COLOR_BTNSHADOW):pDC->GetNearestColor(DarkenColor(70,color3DShadow));

  if(bHighContrast)
  {
    color3DShadow = FxGetSysColor(COLOR_BTNTEXT);
    color3DHilight = FxGetSysColor(COLOR_BTNTEXT);
  }
  ...
  else
  {
    ...
    if(!strText.IsEmpty())
    {
      COLORREF crText = FxGetSysColor(COLOR_MENUTEXT);
      ...
      else
      {
        if(lpDIS->itemState&ODS_SELECTED)
        {
          crText = FxGetSysColor(COLOR_HIGHLIGHTTEXT);
        }
...
void CNewMenu::DrawItem_OldStyle (LPDRAWITEMSTRUCT lpDIS, BOOL bIsMenuBar)
{
  ...
  // Fx mod
  // COLORREF colorBack = bIsMenuBar?GetMenuBarColor():FxGetSysColor(COLOR_MENU);
  COLORREF colorBack = bIsMenuBar?GetMenuBarColor():FxGetSysColor(COLOR_WINDOW);
  ...
  else
  {
    ...
    // set some colors and the font
    m_brSelect.CreateSolidBrush(FxGetSysColor(COLOR_HIGHLIGHT));
    ...
    {
      pDC->Draw3dRect(rect,FxGetSysColor(COLOR_3DHILIGHT),FxGetSysColor(COLOR_3DSHADOW));
    }
    else if (bDrawSmallSelection)
    {
      pDC->DrawFocusRect(rect);
      ...
      if(checkflag||standardflag||selectedflag||disableflag)
      {
        ...
        else
        {
          ...
          if (IsNewShell())
          {
            if(state&ODS_CHECKED)
            {
              pDC->Draw3dRect(IconRect,FxGetSysColor(COLOR_3DSHADOW),FxGetSysColor(COLOR_3DHILIGHT));
            }
            else if (selectedflag)
            {
              pDC->Draw3dRect(IconRect,FxGetSysColor(COLOR_3DHILIGHT),FxGetSysColor(COLOR_3DSHADOW));
            }
          }
...
//          pDC->DrawState(ptImage, size, hDrawIcon, DSS_DISABLED, (HBRUSH)NULL);
    ...
    if(!strText.IsEmpty())
    {
      ...
      {
        if(lpDIS->itemState&ODS_INACTIVE)
          crText = FxGetSysColor(COLOR_GRAYTEXT);
      }
      else
      {
        if(lpDIS->itemState&ODS_SELECTED)
      ...
      else
      {
        ...
        if(!(state & ODS_SELECTED))
        {
          ...
          pDC->SetTextColor(FxGetSysColor(COLOR_BTNHILIGHT));
          ...
          pDC->SetTextColor(FxGetSysColor(COLOR_GRAYTEXT));
          pDC->DrawText(leftStr,RectText, nFormat);
          if(tablocr!=-1)
          {
            pDC->DrawText(rightStr,RectText,nFormatr);
...
void CNewMenu::DrawSpecial_OldStyle(CDC* pDC, LPCRECT pRect, UINT nID, DWORD dwStyle)
{
  ...
  {
    oldColor = pDC->SetTextColor(::FxGetSysColor(COLOR_HIGHLIGHTTEXT));
  }
  else
  {
    oldColor = pDC->SetTextColor(::FxGetSysColor(COLOR_MENUTEXT));
  }
      ...
      if(pMenu && pMenu->GetParent())
      {
        ...
        if(pParentMenu && !pParentMenu->IsPopup())
        {
          ...
          if(!Rect.IsRectEmpty())
          {
            ...
            if (NumScreenColors() > 256)
            {
              colorSmalBorder = MixedColor(CNewMenu::GetMenuBarColor(),FxGetSysColor(COLOR_WINDOW));
            }
            else
            {
              colorSmalBorder = FxGetSysColor(COLOR_BTNFACE);
            }
...
void CMenuTheme::DrawShade( HWND hWnd, HDC hDC, CPoint screen)
{
  ...
  if (NumScreenColors() <= 256)
  {
    DWORD rgb = ::FxGetSysColor(COLOR_BTNSHADOW);
...
BOOL CMenuTheme::OnEraseBkgnd(HWND hWnd, HDC hDC)
{
  ...
  pDC->FillSolidRect (Rect,CNewMenu::GetMenuColor(0));//FxGetSysColor(COLOR_MENU));
...
BOOL CMenuTheme::OnDrawBorder(HWND hWnd, HDC hDC, BOOL bOnlyBorder)
{
  ...
  if(DoDrawBorder() && pData)
  {
    ...
    if(!IsShadowEnabled())
    {
      ...
      if(bHighContrast)
      {
        pDC->Draw3dRect(CRect(CPoint(0,0),CSize(winW-4,winH-4)),FxGetSysColor(COLOR_BTNTEXT ),FxGetSysColor(COLOR_BTNTEXT ));
      }
      else
      {
        pDC->Draw3dRect(CRect(CPoint(0,0),CSize(winW-4,winH-4)),FxGetSysColor(COLOR_HIGHLIGHT),FxGetSysColor(COLOR_HIGHLIGHT));
    ...
    else
    {
      ...
      if(bHighContrast)
      {
        pDC->Draw3dRect(CRect(CPoint(0,0),CSize(winW-0,winH-0)),FxGetSysColor(COLOR_BTNTEXT ),FxGetSysColor(COLOR_BTNTEXT ));
      }
      else
      {
        pDC->Draw3dRect(CRect(CPoint(0,0),CSize(winW-0,winH-0)),FxGetSysColor(COLOR_HIGHLIGHT),FxGetSysColor(COLOR_HIGHLIGHT));
      }
    }
...
BOOL CMenuThemeXP::OnDrawBorder(HWND hWnd, HDC hDC, BOOL bOnlyBorder)
{
  ...
  if(DoDrawBorder() && pData)
  {
    ...
    COLORREF crWindow = FxGetSysColor(COLOR_WINDOW);
  ...
  case WM_ERASEBKGND:
    {
      CRect rect;
      GetClientRect(hWnd,&rect);
      COLORREF blendColor = ::FxGetSysColor(COLOR_ACTIVECAPTION);//RGB(255,0,0);
