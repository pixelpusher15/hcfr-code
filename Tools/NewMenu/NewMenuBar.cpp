      ...
      if(m_nState&ODS_HOTLIGHT)
      {
        if(m_nState&ODS_SELECTED)
        {
		  ...
		  if(fxUseCustomColor)
		  {
			...
			DrawGradient(pDC,rect,colorSel,LightenColor(50,colorSel),false);
		  }
		  else
			DrawGradient(pDC,rect,RGB(242,151,107),RGB(249,202,163),false);
		  // end of fx mod
  ...
  m_hWndCommand(NULL)
{
  ...
  // Start of Fx mod
//  if(fxDrawMenuBorder)
  m_cyBottomBorder = 2;	// Fx mod: was 1. Increased to 2 to allow correct border drawing
//  else
//	  m_cyBottomBorder = 1;
  // End of fx mod
}
...
void CNewMenuBar::DrawBorders(CDC* pDC, CRect& rect)
{
  return;
}
